...
void prep_momentum_response_tx(momentum_frame_t request) {
  ...
  sensor_data_t data = {bno085_quaternion_i,
                        ...
                        gnss_data.course_deg,
                        gnss_data.magnetic_deg,
                        gnss_data.mag_dir,
  ...
  case MOMENTUM_FRAME_TYPE_GNSS_DATETIME:
    len = build_gnss_datetime_payload(&frame, &data);
    break;
  case MOMENTUM_FRAME_TYPE_GNSS_COORD:
    len = build_gnss_coord_payload(&frame, &data);
    break;
  case MOMENTUM_FRAME_TYPE_GNSS_ALT_SPEED:
    len = build_gnss_altitude_speed_payload(&frame, &data);
    break;
  case MOMENTUM_FRAME_TYPE_GNSS_HEAD:
    len = build_gnss_heading_payload(&frame, &data);
    break;
  case MOMENTUM_FRAME_TYPE_GNSS_STATS:
    len = build_gnss_stats_payload(&frame, &data);
    break;
  default:
    len = 0;
    break;
...
void process_reaction(momentum_frame_t request) {
  ...
  case MOMENTUM_FRAME_TYPE_RESET:
    NVIC_SystemReset(); // Software driven NRESET.
    break;              // Redundant.
